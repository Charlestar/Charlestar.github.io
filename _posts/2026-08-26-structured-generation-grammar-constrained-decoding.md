...
上式只表示对当前分布做硬 Mask 后的局部归一化，要求分母大于零。若随后还有 temperature、top-k/top-p，最终采样分布还会改变；若把这些处理预先计入 $p$，又必须确认所用次序与实际实现相同。合法集合非空但其概率全部为零时，同样不能做零除。

还要区分“每一步都合法”与“从原模型在完整合法字符串上的条件分布采样”。设合法答案只有 `AX`、`BY`，模型首步给 A/B 各 0.5，后续给 X\|A 的概率为 0.9、Y\|B 为 0.1。逐步 Mask 会先以 0.5/0.5 选 A/B，再强制接 X/Y，最终两个答案各占 0.5；原模型在两条合法完整答案上的归一化概率却是 0.9/0.1。普通约束解码保证语言有效性，不自动保持全序列条件分布。
...
这里的 Regex 指后端支持的**正规语言子集**，可转换成有限状态自动机；不能把包含任意反向引用等扩展的全部 PCRE/Python regex 都视为等价 DFA。有限状态机只需记录当前状态，适合不需要任意深度嵌套的规则。
...
只有当前 Grammar 状态已经接受一个完整值时，EOS 才能进入允许集合。若对象还缺少 `}`，提前 EOS 必须被屏蔽。但“可以结束”不必等于“必须结束”：例如 Choice 同时允许 `a` 与 `ab`，读到 `a` 时既可结束，也可继续读 `b`，具体终止策略必须写入接口契约。

与此同时，达到 `max_tokens` 属于外部强制截断，Grammar 无法凭空补齐结构。服务应把这种结果标为 `length` 或 `incomplete`，不能宣称它是有效结构化输出。

### 10.3 字符级续写边界与 Tokenization 未必一致
...
Prompt 编码后当然止于一个 token 边界，但“先单独编码 Prompt，再续写”未必等于“把 Prompt 与完整续写一起编码”：一个更合适的词表 token 可能跨过原来的字符级分界。Token Healing 通常回退边界处的 token，再约束重生成内容必须保留已有字节前缀。若 Grammar 后端和 Token Healing 各自修改前缀，却没有共享同一份字节视图，就可能错误屏蔽本来合法的续写。
...
还要处理“合法集合被其他过滤器清空”的情况。例如 Grammar 只允许一个低概率 Token，而极端 top-k 实现先把它删掉。可靠实现应先求所有硬约束的交集，再在其中应用概率过滤。若仅是软过滤清空集合，可以按事先约定撤回软过滤；若 Grammar 与其他禁止规则本身没有交集，则必须报错，不能通过只保留 Grammar 而绕过其他硬约束。
...
一种直接的实现是让 Draft 与 Target 从同一正式前缀创建彼此隔离的 Grammar 状态快照：Draft 在每个位置也只从当前合法集合提出 Token，Target 再在相同前缀对应的约束分布下验证，不能共享并随意推进同一个可变 Matcher。假设某个实现没有在 Draft 侧施加约束，因而提出：
...
随后从状态 `S2` 产生替代 Token。这里还要区分解码模式：Greedy 路径选择 Target 在合法集合中的最高分 Token；若随机验证采用经典 $\min(1,p_G/q_{proposal})$ 接受规则，则拒绝后必须使用与之配对的校正分布

$$
\operatorname{norm}\!\left(\max(0,\ p_G-q_{proposal})\right)
$$

采样，其中 $p_G$ 是状态 `S2` 下经过 Grammar 约束及指定采样处理的 Target 分布，$q_{proposal}$ 是 Draft 实际用于提出该位置 Token 的分布；若 Draft 也正确施加了同一约束，它就是对应的 $q_G$。在这套接受规则下，不能丢弃候选后直接从 $p_G$ 重采样，否则一般会改变目标分布。另一种精确规则可以独立从 $p_G$ 采样并比较候选，在首次失配时保留**同一次 target 抽样结果**；它不使用上述残差，但不能与经典接受率混搭。无论哪种规则，都不能先接受整段 Draft，再用最终字符串做 Grammar 校验。
...
6. 模型执行 Prefill
7. 最后一段 Prefill 的 logits 产生首个输出；后续 Decode 同样执行：
