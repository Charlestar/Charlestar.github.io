...
DSpark 不应改变目标验证规则，但“剩余候选仍走标准拒绝采样”本身还不够。调度也可能通过选择性保留候选，改变实际送给验证器的提议分布。

关键条件是 **non-anticipating admission（不窥视候选结果的准入）**：决定是否纳入当前位置时，不能利用依赖该位置已采样 token 的后续置信度，回头筛选这个位置。论文 §3.2.2 的顺序 early-stop 和 §5.2 的滞后容量决策都明确处理了这个因果边界。再加上以下约束，截短才保持相应推测采样的分布保证：
...
论文附录 A 给出一个可复算的反例。目标分布 $p(A)=0.7,p(B)=0.3$，草稿分布 $q(A)=q(B)=0.5$，第一个位置置信度为 $c_1=0.8$。假设处理 1、2、3 个 token 的 steps/s 分别为 $1,0.5,0.45$，窗口效用按“期望提交数 × steps/s”计算：

$$
\Theta_0=1,\qquad \Theta_1=(1+0.8)\times0.5=0.9
$$

若第一枚草稿是 A，下一位置预测为 $c_2=0.9$，则 $\Theta_2=(1+0.8+0.8\times0.9)\times0.45=1.134$；若第一枚是 B，令 $c_2=0$，则 $\Theta_2=0.81$。回看全部长度后取全局最大值，就只在抽到 A 时保留草稿，抽到 B 时选择零候选并直接从目标采样。A 草稿的标准接受概率仍为 $\min(1,0.7/0.5)=1$，最终却有：

$$
P(output=A)=0.5+0.5\times0.7=0.85\ne0.7
$$

错误不在拒绝公式，而在准入已经筛掉了一类提议。顺序 early-stop 在 $\Theta_1\le\Theta_0$ 时停止，不会再用 $c_2$ 推翻前面的决定。early-stop 的全局最优性还依赖论文的单峰条件；真实阶梯成本若不满足，不能同时宣称“任意全局搜索都无偏”和“early-stop 总是最优”。[DSpark 论文 §3.2.2、§5.2、附录 A](https://arxiv.org/html/2607.05147v1)
...
论文 §5.2 用两步之前的置信度预测确定本轮**聚合验证容量 $K$**；当前候选仍使用当前置信度进行排序和动态 Top-K 分配。不能把它理解为所有候选评分都来自两步以前。滞后容量既让 CPU 避免等待最新 GPU 输出，也切断了“根据当前样本的未来信息回选容量”的依赖；当前排序仍须满足前面的因果准入与前缀约束。历史预测可能过期，因而容量效率仍需监控。
...
如果仍提交完整验证得到的合法前缀，这才是“不改变执行”的 shadow 阶段。`cap-accept` 也执行完整验证，但会按建议窗口限制提交，改变本轮推进位置；它适合与 `compact` 对齐截短语义并观察被放弃的候选，不能与纯 shadow 混为一谈。
...
其中最危险的不是“没有加速”，而是调度改变目标分布、用错 KV 或违反采样契约。Greedy 路径可做确定性 token/logits 对照；随机路径需要分布检验和明确的 RNG 消费约定。仅设置相同 seed，并不保证不同窗口消耗相同数量、相同顺序的随机数，因此不能把每次文本不同都判为 bug；反之，几次文本相同也不能证明无偏。
