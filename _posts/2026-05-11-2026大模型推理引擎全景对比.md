...
> 本文于 2026-09-09 再次核对官方文档。TensorRT-LLM 的后端移除结论以迁移页对应 commit `c295dd9` 为范围，不适用于仍使用旧 release 的部署。推理引擎变化很快，具体模型、量化和后端支持应在采用时重新查阅支持矩阵。文中不保留脱离测试条件的固定吞吐排名。
...
在线服务还常使用 **goodput**。一种口径是按请求检查 TTFT、TPOT 阈值，只计满足全部条件且成功完成的请求或其输出 token；另一种口径是扫描负载，寻找整体 P99 TTFT、P99 TPOT 和错误率都达标时的最大到达率。这两者不能混为同一个数：P99 达标允许少量请求超阈值，不表示每条请求都合格。报告必须说明计数单位、阈值和失败/取消请求的处理方式。

这里的 TPOT 指一条请求首 token 到末 token 的耗时除以输出数减一；输出数不超过 1 时不定义该平均间隔。客户端流式 chunk 也未必恰好包含一个 token，不能直接把 chunk 间隔当作模型的逐 token ITL。
