...
假设系统当前正在处理100个解码任务，每个任务每步生成1个token。同时，有10个新的预填充请求到达，每个请求的prompt长度为1000 tokens。在stall-free batching策略下，这10个预填充请求会被分割成1000个chunk（每个chunk处理1个token），并与现有的100个解码任务混合执行。

如果GPU的最大批处理容量为128，那么每个step只能处理128个请求。这意味着每个预填充chunk平均需要等待约8个step才能被执行（100解码任务 + 1000预填充chunks ≈ 1100 total，1100/128 ≈ 8.6 steps）。对于长度为1000的prompt，总的等待时间将是巨大的，远远超过合理的TTFT SLO。
