...
下面先假设词表有限、候选概率为正，且 top-k 边界没有同分项，$1\le k\le V$。每次拒绝的样本都严格低于保留边界，因此提升 pivot 不会删掉真正的 top-k。零概率项可以先排除；边界 ties 则需要后文所述的明确选择规则，不能直接套用这个严格不等式证明。
...
单 pivot sampler 在常见分布上往往几轮就结束，但缺少与词表规模无关的小轮数保证。它并非完全没有有限上界：在前述严格边界假设下，每次拒绝至少移除一个非 top-k 项，最多经过 $V-k$ 次拒绝，再接受一次，即至多 $V-k+1$ 轮。大词表下这个最坏上界仍很大；若连续抽到只比当前 pivot 略高的坏 token，许多候选可能迟迟没有被排除。
...
分别验证：相同状态可重放、offset 按实现约定推进而不复用随机数区间、batch row 重排不会串用每个 request 的 RNG 流、CUDA Graph replay 时 seed tensor 能生效。独立随机样本仍可能选中相同 token，不能用“输出必须不同”判断 offset 是否正确。
