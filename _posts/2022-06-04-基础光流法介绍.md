...
稀疏与稠密描述的是**在哪些位置输出运动估计**，不是约束方程本身的唯一分类。下面分别以常见的稀疏 LK 跟踪和稠密 Farnebäck 实现说明。
...
其中二次项的三个矩阵维度依次为 $1\times2$、$2\times2$、$2\times1$，结果是标量。展开后两个非对角项合成 $r_6xy$，所以它与上一行的 $r_4x^2+r_5y^2+r_6xy$ 一致。
