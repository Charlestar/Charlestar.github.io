...
常见比喻是助理先写草稿、专家批量审稿，但随机采样下必须明确“怎样接受、怎样替换”这一整套概率规则。本文推导经典的 $\min(1,p/q)$ 接受方案；正残差修正是与这套接受规则配对的条件，不是所有精确验证算法都必须采用的唯一形式。
...
在本文的 $\min(1,p/q)$ 接受方案中，这也解释了两个看似合理但错误的简化：

- 拒绝后直接从完整 $p$ 采样，会重复分配接受路径已经覆盖的概率质量；
- 只比较草稿与目标的 argmax，不能保持有温度采样时的目标分布。

另一种精确但通常接受率更低的方案，是在每个位置独立采样 $Y\sim q$、$Z\sim p$，相同时继续，不同时直接提交**这一次已经采出的 $Z$**并停止验证。这一位置无论是否匹配最终都输出 $Z$，所以分布仍为 $p$；它的匹配率是 $\sum_xp(x)q(x)$，而非上面的 $\sum_x\min(p(x),q(x))$。失配后若丢弃 $Z$ 再重新采样，或把这套匹配判定与经典残差规则混用，就不再有这个证明。相关实现示例见 [Transformers v4.57.1 的 assisted generation](https://github.com/huggingface/transformers/blob/v4.57.1/src/transformers/generation/utils.py)。
...
真实接受事件并不独立，这个例子只说明边际收益会下降。最佳 $k$ 取决于 prompt、领域、采样温度、batch 和硬件。动态 speculative decoding 可以根据此前轮次的接受情况或系统负载调整候选长度。若长度选择会查看本轮已抽到但尚未接纳的候选，则必须重新核查选择偏差：事后只保留“看起来容易接受”的候选，不自动满足上述 $Y\sim q$ 的条件分布。应采用具有非前瞻性保证的停止规则，或为选择后的 proposal 另行证明正确性，详见 [DSpark 的 §3.2.2 与附录 A](https://arxiv.org/html/2607.05147v1)。
