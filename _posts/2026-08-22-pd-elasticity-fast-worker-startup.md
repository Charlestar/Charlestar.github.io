...
这是一份阶段清单和串行基线，不是所有实现的 wall-clock 恒等式。权重读取、反序列化、H2D 可以按 Chunk 流水，部分运行时初始化也可能并行；实际 `T_ready` 应取启动依赖 DAG 的最长路径，同时考虑共享带宽和资源竞争。把有重叠的阶段时长直接相加，会重复计算已经被覆盖的时间。
