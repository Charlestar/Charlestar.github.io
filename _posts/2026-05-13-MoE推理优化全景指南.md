...
这里 $\operatorname{TopK}(s)$ 指按分数选出的 **$k$ 个专家索引**，平分时使用确定的 tie-breaking；不是“分数等于某个 Top-k 值的所有专家”。$p_i$ 的归一化、缩放、分组限制与 shared expert 是否另加，都必须按具体模型定义，不能把一个示例的权重处理套到所有 MoE checkpoint。
...
该比值只在 $N_a>0$ 时定义；空 batch 应记录为无观测值，而不是进行零除。Assignment 数也不等于网络发送行数：同一 token 的多个专家若位于同一目的 rank，通信实现可以先去重发送，再在目的端展开。
